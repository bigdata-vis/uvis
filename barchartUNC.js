...
{
    let _chart = new Object();
    let _keyXAxe = "Name";
    let _variables = $.map(data[0], function(element, index) {
        if(_keyXAxe != index )
            return {name:index, color:false};
    });
    let _displayedVariables = _variables;
    let _axeXData = $.map(data, function(element, index) {
        if(_keyXAxe )
            return element[_keyXAxe];
    });
    let _useRightYAxis = false;
    let _displayUncertainty = true;
